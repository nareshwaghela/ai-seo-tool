...
def get_related_searches(keyword):
    try:
        ...
        if r.status_code == 200:
            soup = BeautifulSoup(r.text, "html.parser")
            related = []
            for div in soup.find_all("div", class_="wCb7ib"):
...
def get_people_also_ask(keyword):
    try:
        ...
        if r.status_code == 200:
            soup = BeautifulSoup(r.text, "html.parser")
            questions = []
            for div in soup.find_all("div", attrs={"class": re.compile("related-question-pair")}):
                text = div.get_text(strip=True)
...
def get_long_tail_variations(keyword):
    prefixes = ["how to", "best way to", "why", "what is", "top 10", "guide to", "tips for", "benefits of", "vs", "review", "for beginners", "free", "online", "step by step"]
    suffixes = ["guide", "tutorial", "tips", "tricks", "for beginners", "step by step", "in 2024", "online", "free"]
    variations = set()
...
def scrape_serp(keyword):
    ...
    try:
        ...
        # Organic Results
        results = []
        # Google changes classes often. Fallback approach checking for h3 inside 'g' div
...
def generate_article_template(keyword, tone, length, extra=""):
    ...
    sections = [f"Understanding {ck}", f"Key Benefits of {ck}", f"How to Implement {ck}", f"Common Mistakes to Avoid", f"Advanced Strategies", f"Measuring Success", f"Future Trends", f"Expert Insights", f"Case Studies", f"Tools Needed", f"FAQs", f"Conclusion"][:num_sections]
    
    html = f'<h2>{ck}: The Complete Guide</h2><p>Are you looking to master {ck}? In this comprehensive guide, we cover everything you need to know.</p>'
    if extra.strip(): html += f"<p><em>Note: {extra}</em></p>"
    
    for sec in sections:
        html += f'<h3>{sec}</h3><p>When it comes to {ck}, understanding the core principles is essential. Experts recommend a structured approach to maximize results and avoid common pitfalls.</p><ul><li>Focus on fundamentals first</li><li>Apply through practical exercises</li><li>Continuously refine your strategy</li></ul>'
    
    html += f'<h3>Conclusion</h3><p>Mastering {ck} takes time, but by following the steps above, you will see significant improvements. Start today and take action!</p>'
    return html
...
page_config = {
    ...
    "SERP": {"title": "🔎 SERP Analysis", "desc": "Scrape top 10 Google results, PAA, and Related Searches for any keyword."},
...
# =========================
    ...
    with st.container():
        ...
        st.caption("⚠️ Note: Google frequently blocks automated scraping. If results are empty, Google might have triggered a CAPTCHA from this server IP.")
    ...
    if run_serp:
        ...
        if total_res is None:
            st.error("Failed to fetch Google results. Google might have blocked the request (CAPTCHA triggered).")
        else:
            # Metrics
            ...
            with m2: st.markdown(f'<div class="metric-card"><div class="metric-label">Organic URLs Found</div><div class="metric-value">{len(organic)}</div></div>', unsafe_allow_html=True)
            ...
            if not organic:
                st.warning("No organic results extracted. Google might have changed its HTML structure or blocked the request.")
            else:
                for i, res in enumerate(organic):
                    st.markdown(f'''
                    <div class="serp-card">
                        <div style="display:flex; align-items:start;">
                            <div class="serp-rank">#{i+1}</div>
                            <div style="flex:1;">
                                <a href="{res['url']}" target="_blank" class="serp-url">{res['url'][:80]}...</a>
                                <div class="serp-title">{res['title']}</div>
                                <div class="serp-desc">{res['desc'][:200]}...</div>
                            </div>
                        </div>
                    </div>
                    ''', unsafe_allow_html=True)
            ...
            if paa:
                ...
                for q in paa:
                    st.markdown(f'<div class="paa-box">👉 {q}</div>', unsafe_allow_html=True)
            ...
            if related:
                ...
                for i, rel in enumerate(related[:9]):
                    with rel_cols[i % 3]:
                        st.markdown(f'<div class="small" style="margin-bottom:8px;">• {rel}</div>', unsafe_allow_html=True)
...
# =========================
    ...
    with st.container():
        ...
        c1, c2 = st.columns(2)
        with c1:
            get_suggest = st.checkbox("Suggestions", value=True)
            get_rel = st.checkbox("Related", value=True)
        with c2:
            get_paa = st.checkbox("PAA", value=True)
            get_lt = st.checkbox("Long-tail", value=True)
