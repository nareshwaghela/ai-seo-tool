...
<style>
    ...
    .g-wrapper-mobile {
        max-width: 380px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; padding: 15px; background: #fff;
    }
    .g-wrapper-desktop {
        max-width: 650px; margin: 0 auto; border: 1px solid #ddd; border-radius: 8px; padding: 20px; background: #fff;
    }
    ...
    .g-title { color: #1a0dab; font-size: 20px; font-weight: 400; line-height: 1.3; margin: 4px 0 2px 0; cursor: pointer; font-family: arial, sans-serif; }
    .g-title:hover { text-decoration: underline; }
    .g-wrapper-mobile .g-title { font-size: 18px; }
    ...
    .g-sitelinks-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 12px 20px; margin-top: 14px; padding-top: 10px; border-top: 1px solid #ebebeb; }
...
with col_form:
    ...
    # Live Title Counter
    t_len = len(st.session_state.get("title", ""))
    st.markdown(f'<div class="input-label">SEO Title Tag <span class="{get_color_css(t_len, 60, 50)}">{t_len} / 60</span></div>', unsafe_allow_html=True)
    ...
    d_len = len(st.session_state.get("desc", ""))
    st.markdown(f'<div class="input-label">Meta Description <span class="{get_color_css(d_len, 160, 140)}">{d_len} / 160</span></div>', unsafe_allow_html=True)
    ...
    with st.expander("Optional: Advanced Snippets", expanded=False):
        # FIXED: Removed unsupported format string
        date_input = st.date_input("Published Date", value=None)
...
with col_preview:
    ...
    # Format Date to "MMM DD, YYYY" safely in Python
    date_str = ""
    if date_input:
