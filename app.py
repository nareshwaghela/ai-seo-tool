...
def get_related_searches(keyword):
    """Google Related Searches scraping"""
    related = []
    try:
        headers = {
            "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/122.0 Safari/537.36"
...
def estimate_difficulty(keyword):
    """Estimate keyword difficulty based on basic signals"""
    words = keyword.split()
...
# =========================
def generate_article_openai(keyword, tone, length, api_key):
...
def generate_article_template(keyword, tone, length):
    """Generate article using template (no API needed)"""
    
    # Clean keyword for title
    ...
    content_templates = {
        "Professional": [
            f"In today's competitive landscape, understanding {clean_keyword} has become essential for professionals and businesses alike. This section explores the foundational concepts that form the backbone of {clean_keyword} and why it matters now more than ever.\n\n<p>{clean_keyword} represents a significant shift in how we approach [relevant field]. Organizations that have embraced this concept have seen measurable improvements in their outcomes.</p>",
            f"<p>When implemented correctly, {clean_keyword} offers numerous advantages that can transform your approach. Let's examine the most significant benefits:</p>\n<ul>\n<li><strong>Increased Efficiency:</strong> Streamlined processes lead to better resource utilization</li>\n<li><strong>Cost Reduction:</strong> Optimized workflows reduce unnecessary expenses</li>\n<li><strong>Improved Outcomes:</strong> Data-driven decisions yield superior results</li>\n<li><strong>Competitive Advantage:</strong> Early adopters gain significant market positioning</li>\n<li><strong>Scalability:</strong> Proven frameworks allow for sustainable growth</li></ul>",
            f"<p>Successful implementation of {clean_keyword} requires adherence to established best practices:</p>\n<ul>\n<li>Start with a clear strategy and defined objectives</li>\n<li>Gather and analyze relevant data before making decisions</li>\n<li>Implement changes incrementally with proper testing</li>\n<li>Monitor performance metrics continuously</li>\n<li>Iterate and optimize based on results</li>\n<li>Document learnings for future reference</li></ul>",
            f"<p>While pursuing {clean_keyword}, many practitioners fall into common pitfalls. Being aware of these can save significant time and resources:</p>\n<ul>\n<li><strong>Skip Research:</strong> Not investing time in understanding the fundamentals</li>\n<li><strong>Rush Implementation:</strong> Moving too quickly without proper planning</li>\n<li><strong>Ignore Data:</strong> Making decisions based on assumptions rather than evidence</li>\n<li><strong>Neglect Training:</strong> Failing to equip team members with necessary skills</li>\n<li><strong>Set Unrealistic Goals:</strong> Expecting overnight transformation</li></ul>",
            f"<p>Implementing {clean_keyword} effectively requires a structured approach. Here's a proven framework:</p>\n<ul>\n<li><strong>Phase 1 - Assessment:</strong> Evaluate current state and identify gaps</li>\n<li><strong>Phase 2 - Planning:</strong> Develop a detailed implementation roadmap</li>\n<li><strong>Phase 3 - Execution:</strong> Implement changes according to plan</li>\n<li><strong>Phase 4 - Monitoring:</strong> Track progress against defined KPIs</li>\n<li><strong>Phase 5 - Optimization:</strong> Refine approach based on insights</li></ul>",
        ]
    }
    ...
    # Introduction
    article_html += f'''<p>Are you looking to understand {clean_keyword} and how it can benefit you? You've come to the right place. In this comprehensive guide, we'll explore everything you need to know about {clean_keyword}, from the basics to advanced strategies.</p>
    
<p>Whether you're a beginner just starting out or an experienced professional looking to deepen your knowledge, this article will provide valuable insights that you can apply immediately.</p>'''
    ...
    for i, section in enumerate(sections):
        ...
        if i < len(content_templates.get(tone, [])):
            ...
            # Generic content
            article_html += f'''<p>When it comes to {clean_keyword}, there are several important factors to consider. Understanding these elements will help you make better decisions and achieve superior outcomes.</p>

<p>Many experts in the field recommend taking a systematic approach to {clean_keyword}. This means breaking down complex concepts into manageable pieces and addressing them one at a time.</p>

<ul>
<li>Focus on understanding the core principles first</li>
<li>Apply what you learn through practical exercises</li>
...
# MAIN UI
# =========================
st.markdown('<div class="main-title">🚀 Free SEO Tool Box</div>', unsafe_allow_html=True)
st.markdown('<div class="sub-title">DA PA Checker • Keyword Research • Article Writer — All in one place, 100% free.</div>', unsafe_allow_html=True)

tab1, tab2, tab3 = st.tabs(["📈 DA PA Checker", "🔍 Keyword Research", "✍️ Article Writer"])

# =========================
# TAB 1: DA PA CHECKER
# =========================
with tab1:
    st.markdown('<div class="tab-title">📈 Domain & Page Authority Checker</div>', unsafe_allow_html=True)
    ...
    if run:
        ...
        st.caption("⚠️ This is an estimated DA/PA checker. It does not return official Moz scores.")

# =========================
# TAB 2: KEYWORD RESEARCH
# =========================
with tab2:
    st.markdown('<div class="tab-title">🔍 Keyword Research Tool</div>', unsafe_allow_html=True)
    st.markdown('<p style="color:#aaa; margin-bottom:20px;">Google Autocomplete, Related Searches, Long-tail variations — all free, no API needed.</p>', unsafe_allow_html=True)
    ...
    if run_kw:
        ...
        if not all_keywords:
            ...
            with col_exp2:
                ...
                st.download_button(
                    label="📥 Download TXT (Keywords Only)",
                    data=txt_data,
            ...
            st.caption("⚠️ Volume and difficulty are estimated values, not from any paid tool.")

# =========================
# TAB 3: ARTICLE WRITER
# =========================
with tab3:
    st.markdown('<div class="tab-title">✍️ AI Article Writer</div>', unsafe_allow_html=True)
    st.markdown('<p style="color:#aaa; margin-bottom:20px;">Generate SEO-optimized articles with or without OpenAI API.</p>', unsafe_allow_html=True)
    ...
    if run_art:
        ...
        with st.spinner("Generating article..."):
            if "OpenAI" in col_mode:
                article_html = generate_article_openai(article_keyword, article_tone, article_length, api_key)
            else:
                article_html = generate_article_template(article_keyword, article_tone, article_length)
        ...
        if article_html.startswith("Error:"):
            ...
            st.caption("⚠️ Template mode generates structured articles. For higher quality, use OpenAI API mode.")
...
<div style="text-align:center; color:#666; font-size:13px;">
    <p>Built with ❤️ | Free SEO Tool Box — No API keys required for DA PA & Keyword Research</p>
    <p>DA/PA are estimated scores, not official Moz metrics. Volume/Difficulty are approximations.</p>
</div>
""", unsafe_allow_html=True)
